...
Riemann sphere, the complex plane, or the unit disk. This serves as the main ingredient for [classification of Riemann surfaces](article:handwave/classification.hw.md).

The proof strategy loosely follows the strategy in _Hubbard, J. H. – Teichmüller theory and applications to geometry, topology and dynamics_, albeit with some modifications. It first turns
simple connectedness into vanishing first real de Rham cohomology. The
noncompact case is then proved by exhausting the surface with simply-connected (technically cohomology zero) domains and constructing Green's functions on them, which can then be used to construct holomorphic maps to discs with increasing radii. These holomorphic maps are shown to converge to a holomorphic map either to the disc or the plane. The compact case is reduced to the noncompact theorem by deleting one
point.
...
Most of the uniformization proof works under the assumption of vanishing 1st-degree de Rham cohomology. In order to have a result that is formulated using simple connectedness of the surface, we need to show that simple connectedness is stronger.
