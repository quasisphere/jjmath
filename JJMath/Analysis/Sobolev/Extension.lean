...
theorem euclideanSobolev_unit_ball_radial_reflection_annulus_pullback_weakDerivative
  ...
  have hw_punct_local : ∀ K : Set H, IsCompact K → K ⊆ Ω₀ →
      MemLp w 2 (MeasureTheory.volume.restrict K) := by
    intro K _hK hKΩ₀
    exact hw_punct.mono_measure (Measure.restrict_mono hKΩ₀ le_rfl)
  have hdw_punct_local : ∀ K : Set H, IsCompact K → K ⊆ Ω₀ →
      MemLp dw 2 (MeasureTheory.volume.restrict K) := by
    intro K _hK hKΩ₀
    exact hdw_punct.mono_measure (Measure.restrict_mono hKΩ₀ le_rfl)
  simpa [U, Ω₀, R] using
      ...
      (by
        simpa [U, Ω₀, R] using
          euclideanSobolevUnitBallRadialReflection_qmp_punctured_annulus
            (H := H))
      hweak_punct hw_punct_local hdw_punct_local
...
%%handwave
name:
  Weighted integrability of a spherical trace
statement:
  A trace $\tau$ has the required weighted unit-sphere integrability relative to a smooth test function $\varphi$ and direction $v$ when both $\theta\mapsto\tau(\theta)$ and $\theta\mapsto\tau(\theta)\varphi(\theta)\langle\theta,v\rangle$ are integrable for spherical measure.
-/
