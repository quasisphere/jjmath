...
%%handwave
name:
  Affine coordinate domain of the standard simplex
statement:
  The affine coordinate domain of $\Delta^k$ is $\{x\in\mathbb R^k:x_i\ge0,\ \sum_i x_i\le1\}$.
-/
...
%%handwave
name:
  Last-coordinate simplex slice map
statement:
  The slice map $\mathbb R^k\times\mathbb R\to\mathbb R^{k+1}$ sends $(x,t)$ to $(x_0,\ldots,x_{k-1},t)$.
-/
...
%%handwave
name:
  Sliced affine simplex domain
statement:
  The sliced domain for $\Delta^{k+1}_{\mathrm{aff}}$ consists of $(x,t)$ with $x\in\Delta^k_{\mathrm{aff}}$ and $0\le t\le1-\sum_i x_i$.
-/
...
%%handwave
name:
  Measurable last-coordinate slicing equivalence
statement:
  Appending the last coordinate gives a measurable equivalence $\mathbb R^k\times\mathbb R\cong\mathbb R^{k+1}$.
-/
...
%%handwave
name:
  Arbitrary-coordinate simplex insertion
statement:
  For $i\in\{0,\ldots,k\}$, the insertion map places $t$ in coordinate $i$ of $x\in\mathbb R^k$ and shifts the remaining coordinates to obtain a vector in $\mathbb R^{k+1}$.
-/
...
%%handwave
name:
  Measurable arbitrary-coordinate insertion equivalence
statement:
  Inserting a distinguished coordinate at position $i$ gives a measurable equivalence $\mathbb R^k\times\mathbb R\cong\mathbb R^{k+1}$.
-/
...
%%handwave
name:
  Affine-to-barycentric simplex coordinate map
statement:
  Affine coordinates $x\in\mathbb R^k$ determine barycentric coordinates $(x_0,\ldots,x_{k-1},1-\sum_i x_i)$ in $\mathbb R^{k+1}$.
-/
...
%%handwave
name:
  Barycentric-to-affine simplex coordinates
statement:
  A barycentric vector $(y_0,\ldots,y_k)$ is sent to its first $k$ entries, regarded as affine simplex coordinates.
-/
...
%%handwave
name:
  Linear barycentric-to-affine projection
statement:
  The continuous linear projection $\mathbb R^{k+1}\to\mathbb R^k$ retains the first $k$ barycentric coordinates.
-/
...
%%handwave
name:
  Linear part of the affine-to-barycentric coordinate map
statement:
  The derivative of $x\mapsto(x,1-\sum_i x_i)$ sends $h$ to $(h_0,\ldots,h_{k-1},-\sum_i h_i)$.
-/
...
%%handwave
name:
  Barycentric coordinate-sum functional
statement:
  The ambient simplex sum functional on $\mathbb R^{k+1}$ is $y\mapsto\sum_{j=0}^k y_j$.
-/
...
%%handwave
name:
  Tangent hyperplane of the barycentric simplex
statement:
  The tangent space to the affine hyperplane containing the barycentric simplex is $\{v\in\mathbb R^{k+1}:\sum_jv_j=0\}$.
-/
...
%%handwave
name:
  Affine coordinates on the barycentric tangent hyperplane
statement:
  The map $h\mapsto(h_0,\ldots,h_{k-1},-\sum_i h_i)$ is a linear equivalence from $\mathbb R^k$ to the zero-sum hyperplane in $\mathbb R^{k+1}$.
-/
...
%%handwave
name:
  Ambient linear map induced by a vertex map
statement:
  A map $f:\{0,\ldots,k\}\to\{0,\ldots,l\}$ induces a continuous linear map on barycentric spaces by summing source coordinates over the fibers of $f$.
-/
...
%%handwave
name:
  Affine coordinate map of a simplex face
statement:
  The affine coordinate map of the $i$-th face converts affine coordinates on $\Delta^k$ to barycentric coordinates, inserts a zero at vertex $i$, and projects to affine coordinates on $\Delta^{k+1}$.
-/
...
%%handwave
name:
  Linear part of a simplex face coordinate map
statement:
  The derivative of the affine $i$-th face map is obtained by passing to the barycentric tangent hyperplane, applying the ambient face inclusion, and projecting back to affine coordinates.
-/
...
%%handwave
name:
  Chosen ambient extension of a smooth simplex
statement:
  A $C^r$ singular simplex has a chosen map from the ambient barycentric space to the manifold that is $C^r$ on the standard simplex and agrees there with the simplex.
-/
...
%%handwave
name:
  Vertex-permutation map of the standard simplex
statement:
  A permutation of the $k+1$ vertices acts continuously on $\Delta^k$ by permuting its barycentric coordinates.
-/
...
%%handwave
name:
  Affine-coordinate vertex permutation
statement:
  The affine self-map of $\Delta^k_{\mathrm{aff}}$ induced by a vertex permutation is obtained by converting to barycentric coordinates, permuting them, and retaining the first $k$ coordinates.
-/
...
%%handwave
name:
  Linear part of an affine vertex permutation
statement:
  The derivative of a vertex-permutation map in affine coordinates is obtained by passing to barycentric tangent coordinates, permuting them, and projecting to the first $k$ coordinates.
-/
...
%%handwave
name:
  Vertex reparametrization of a smooth singular simplex
statement:
  A permutation of the vertices of $\Delta^k$ reparametrizes a smooth singular simplex $\sigma$ by precomposition, producing $\sigma\circ p$ with the same regularity.
-/
...
%%handwave
name:
  Inclusion of a standard simplex face
statement:
  The $i$-th face inclusion $\Delta^k\hookrightarrow\Delta^{k+1}$ inserts a zero in barycentric coordinate $i$.
-/
...
%%handwave
name:
  Face of a smooth singular simplex
statement:
  The $i$-th face of a smooth singular $(k+1)$-simplex is its precomposition with the standard face inclusion $\Delta^k\hookrightarrow\Delta^{k+1}$.
-/
...
%%handwave
name:
  Affine parametrization induced by a simplex extension
statement:
  An ambient simplex extension $G:\mathbb R^{k+1}\to M$ induces the affine parametrization $x\mapsto G(x_0,\ldots,x_{k-1},1-\sum_i x_i)$.
-/
...
%%handwave
name:
  Top-degree pullback form along a simplex extension
statement:
  A continuous $k$-form on $M$ pulls back along an ambiently extended $k$-simplex to the alternating $k$-form obtained by evaluating at the affine parametrization and composing with its derivative.
-/
...
%%handwave
name:
  Pullback of a form along an extended simplex
statement:
  A continuous $k$-form on $M$ pulls back along an ambiently extended $m$-simplex to a $k$-form on its affine coordinate domain by composition with the parametrization derivative.
-/
...
%%handwave
name:
  Oriented coefficient of a simplex pullback
statement:
  The coefficient of a pulled-back top-degree $k$-form is its value on the standard ordered coordinate basis of $\mathbb R^k$.
-/
...
%%handwave
name:
  Coordinate basis omitting the last slice direction
statement:
  In $\mathbb R^{k+1}$, the sliced-simplex basis without the last direction is the ordered family of standard basis vectors indexed by the first $k$ coordinates.
-/
...
%%handwave
name:
  Value of an integral over a singular chain
statement:
  The integral of a continuous $k$-form over a smooth singular $k$-chain is the value of the chain-integration linear map, namely the finite integer-weighted sum of the simplex integrals.
-/
