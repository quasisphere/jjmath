...
- Every theorem, lemma, and definition added to the repository must have a `%%handwave` metadata docstring, including supporting results rather than only principal exported results.
- Every `%%handwave` metadata block for a theorem, lemma, or definition must contain `name` and `statement` fields. Theorem and lemma blocks must additionally contain a `proof` field. Do not use a `description` field in place of `statement` for definitions.
- Use `$...$` for inline LaTeX in Handwave metadata and Markdown articles. Do not use `\(...\)`, because Handwave does not render that delimiter form.
- In `%%handwave` metadata fields, write mathematical prose rather than Lean-facing prose. Do not use Lean identifiers in visible metadata text unless the identifier is used as the URL target of a link.
...
- In `%%handwave` metadata, make the statement precise enough to identify the quantified objects and the mathematical object being defined or the main formula being proved. Keep theorem and lemma proof sketches short and make them conform to the actual formal proof: if the proof primarily applies another formal theorem, link to the mathematical conclusion of that theorem using a `lean:` URL.
